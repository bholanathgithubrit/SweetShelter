...
const ImagesSection=()=>{
    ...
    const handelDelete=(event:React.MouseEvent<HTMLButtonElement,MouseEvent>,imageUrl:string)=>{
        ...
        <div>
            ...
            <div className="border rounded p-4 flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-6 gap-4">
                        {existingImageUrls.map((url)=>(
                            <div className="relative group" key={url}>
                                ...
                                <button className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 text-white" onClick={(event)=>handelDelete(event,url)}>Delete</button>
                            </div>
                        ))}
                    </div>
                )}
                <input type="file" multiple accept="image/*" {...register("imageFiles",{validate:(imageFiles)=>{
                    ...
                    else if(totalLength>6)return "you can upload Maximum 6 image"
                    return true
